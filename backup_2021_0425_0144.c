...
bool isLeftAnglePositive[servoCnt];
...
void moveServo(int i, int b)
{  
    leftAngle[i] = b - getAngle[i]; // move angle calcu

    if(leftAngle[i] > 0){ // + route
        isLeftAnglePositive[i] = true;
        timeleft[i] = movePeriod*1000;
        isWaiting[i] = false;
    } else if(leftAngle[i] < 0){ // - route
        isLeftAnglePositive[i] = true;
        timeleft[i] = movePeriod*1000;
        isWaiting[i] = false;
    }
